...
const getFilterValue = getFilterValueFromQuery<NovesHistoryFilterValue>.bind(
  null,
...
const AddressAccountHistory = ({ scrollRef }: Props) => {
  ...
  const [filterValue, setFilterValue] = React.useState<NovesHistoryFilterValue>(
  ...
  const { data, isError, pagination, isPlaceholderData } = useQueryWithPages({
    ...
    scrollRef,
    options: {
      placeholderData: generateListStub<"noves_address_history">(
        NOVES_TRANSLATE,
        10,
        { hasNextPage: false, pageNumber: 1, pageSize: 10 }
      ),
    },
  });
  ...
  const filteredData = isPlaceholderData
    ? data?.items
    : data?.items.filter((i) =>
        filterValue ? getFromToValue(i, currentAddress) === filterValue : i
      );
