...
const AddressTokenTransfers = ({
  ...
  const { isError, isPlaceholderData, data, pagination, onFilterChange } =
    useQueryWithPages({
  ...
  const handleNewSocketMessage: SocketMessage.AddressTokenTransfer["handler"] =
    (payload) => {
      ...
      payload.token_transfers.forEach((transfer) => {
        if (
          data?.items &&
          data.items.length + newItems.length >= overloadCount
        ) {
      ...
      if (newItems.length > 0) {
        queryClient.setQueryData(
          getResourceKey("address_token_transfers", {
            pathParams: { hash: currentAddress },
            queryParams: { ...filters },
          }),
  ...
  const numActiveFilters =
    (filters.type?.length || 0) + (filters.filter ? 1 : 0);
  const isActionBarHidden =
    !tokenFilter && !numActiveFilters && !data?.items.length && !currentAddress;
  ...
  const tokenFilterComponent = tokenFilter && (
    <Flex
      alignItems="center"
      flexWrap="wrap"
      mb={{ base: isActionBarHidden ? 3 : 6, lg: 0 }}
      mr={4}
    >
      <Text whiteSpace="nowrap" mr={2} py={1}>
        Filtered by token
      </Text>
