...
const getTokenFilterValue = getFilterValuesFromQuery<NFTTokenType>.bind(
  null,
  NFT_TOKEN_TYPE_IDS
);
...
const AddressTokens = () => {
  ...
  const displayTypeCookie = cookies.get(
    cookies.NAMES.ADDRESS_NFT_DISPLAY_TYPE,
    useAppContext().cookies
  );
  const [nftDisplayType, setNftDisplayType] = React.useState<TNftDisplayType>(
    displayTypeCookie === "list" ? "list" : "collection"
  );
  const [tokenTypes, setTokenTypes] = React.useState<
    Array<NFTTokenType> | undefined
  ...
  const erc20Query = useQueryWithPages({
    ...
    options: {
      ...
      placeholderData: generateListStub<"address_tokens">(
        ADDRESS_TOKEN_BALANCE_ERC_20,
        10,
  ...
  const collectionsQuery = useQueryWithPages({
    ...
    options: {
      ...
      placeholderData: generateListStub<"address_collections">(
        ADDRESS_COLLECTION,
        10,
        { next_page_params: null }
      ),
  ...
  const nftsQuery = useQueryWithPages({
    ...
    options: {
      ...
      placeholderData: generateListStub<"address_nfts">(ADDRESS_NFT_1155, 10, {
        next_page_params: null,
      }),
    },
    filters: { type: tokenTypes },
  ...
  const handleNFTsDisplayTypeChange = React.useCallback(
    (val: TNftDisplayType) => {
  ...
  const tabs = [
    {
      id: "tokens_erc20",
      title: "ERC-20",
      component: <ERC20Tokens tokensQuery={erc20Query} />,
    },
    {
      id: "tokens_nfts",
      title: "NFTs",
  ...
  if (tab === "tokens_nfts") {
    pagination =
      nftDisplayType === "list"
        ? nftsQuery.pagination
        : collectionsQuery.pagination;
  } else {
