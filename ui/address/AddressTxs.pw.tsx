...
base.describe("socket", () => {
  ...
  test("without overload", async ({ mount, page, createSocket }) => {
    ...
    const channel = await socketServer.joinChannel(
      socket,
      `addresses:${CURRENT_ADDRESS.toLowerCase()}`
    );

    const itemsCount = await page.locator("tbody tr").count();
    expect(itemsCount).toBe(2);

    socketServer.sendMessage(socket, channel, "transaction", {
      transactions: [txMock.base2, txMock.base4],
    });
  ...
  test("with update", async ({ mount, page, createSocket }) => {
    ...
    socketServer.sendMessage(socket, channel, "transaction", {
      transactions: [txMock.base, txMock.base2],
    });
  ...
  test("with overload", async ({ mount, page, createSocket }) => {
    ...
    socketServer.sendMessage(socket, channel, "transaction", {
      transactions: [txMock.base2, txMock.base3, txMock.base4],
    });
  ...
  test("without overload, with filters", async ({
    ...
    const API_URL_WITH_FILTER =
      buildApiUrl("address_txs", { hash: CURRENT_ADDRESS }) + "?filter=from";
