...
const getFilterValue = getFilterValueFromQuery<AddressFromToFilter>.bind(
  null,
...
const AddressTxs = ({ scrollRef, overloadCount = OVERLOAD_COUNT }: Props) => {
  ...
  const [sort, setSort] = React.useState<TransactionsSortingValue | undefined>(
    getSortValueFromQuery<TransactionsSortingValue>(router.query, SORT_OPTIONS)
  );

  const isMobile = useIsMobile();
  const currentAddress = getQueryParamString(router.query.hash);

  const [filterValue, setFilterValue] = React.useState<AddressFromToFilter>(
    getFilterValue(router.query.filter)
  );
  ...
  const handleNewSocketMessage: SocketMessage.AddressTxs["handler"] =
    React.useCallback(
      (payload) => {
        setSocketAlert("");

        queryClient.setQueryData(
          getResourceKey("address_txs", {
            pathParams: { hash: currentAddress },
          ...
          (prevData: AddressTransactionsResponse | undefined) => {
            ...
            payload.transactions.forEach((tx) => {
              const currIndex = prevData.items.findIndex(
                (item) => item.hash === tx.hash
              );
              ...
              if (currIndex > -1) {
                ...
                if (matchFilter(filterValue, tx, currentAddress)) {
                  if (
                    newItems.length + prevData.items.length >=
                    overloadCount
                  ) {
                    newCount++;
                  } else {
            ...
            return {
              ...prevData,
              items: [...newItems, ...prevData.items].sort(
                sortTxsFromSocket(sort)
              ),
            };
          }
        );
  ...
  const channel = useSocketChannel({
    ...
    isDisabled:
      ...
      addressTxsQuery.isPlaceholderData,
  });
