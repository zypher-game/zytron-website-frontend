...
const SolidityscanReport = ({ className, hash }: Props) => {
  ...
  const vulnerabilities =
    data?.scan_report.scan_summary.issue_severity_distribution;
  const vulnerabilitiesCounts = vulnerabilities
    ? Object.values(vulnerabilities)
    : [];
  const vulnerabilitiesCount = vulnerabilitiesCounts.reduce(
    (acc, val) => acc + val,
    0
  );
    ...
    <Popover isOpen={isOpen} onClose={onClose} placement="bottom-start" isLazy>
      ...
      <PopoverContent w={{ base: "100vw", lg: "328px" }}>
        <PopoverBody px="26px" py="20px" fontSize="sm">
          ...
          <LinkExternal href={data?.scan_report.scanner_reference_url}>
            View full report
          </LinkExternal>
        </PopoverBody>
      </PopoverContent>
