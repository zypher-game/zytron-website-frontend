...
const ADDRESSES_LOOKUP_API_URL =
  buildApiUrl("addresses_lookup", { chainId: config.chain.id }) +
  `?address=${ADDRESS_HASH}&resolved_to=true&owned_by=true&only_active=true&order=ASC`;

test("base view", async ({ mount, page }) => {
  await page.route(ADDRESSES_LOOKUP_API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify({
        items: [
  ...
  await expect(page).toHaveScreenshot({
    clip: { x: 0, y: 0, width: 550, height: 350 },
  });
});
