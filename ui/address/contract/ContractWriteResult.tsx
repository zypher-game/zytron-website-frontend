...
const ContractWriteResult = ({
  result,
  onSettle,
}: ResultComponentProps<SmartContractWriteMethod>) => {
  const txHash =
    result && "hash" in result ? (result.hash as `0x${string}`) : undefined;
  const txInfo = useWaitForTransactionReceipt({
    hash: txHash,
  });

  return (
    <ContractWriteResultDumb
      result={result as ContractMethodWriteResult}
      onSettle={onSettle}
      txInfo={txInfo}
    />
  );
