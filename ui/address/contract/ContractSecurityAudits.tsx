...
const ContractSecurityAudits = ({ addressHash }: Props) => {
  ...
  React.useEffect(() => {
    ...
    setHasScroll(
      containerRef.current.scrollHeight >=
        containerRef.current.clientHeight + SCROLL_GRADIENT_HEIGHT / 2
    );
  }, []);
  ...
  return (
    <>
      <Button variant="outline" size="sm" onClick={modalProps.onOpen}>
        Submit audit
      </Button>
      {data?.items && data.items.length > 0 && (
        <Box position="relative">
          ...
          >
            {data.items.map((item) => (
              <LinkExternal
                href={item.audit_report_url}
                key={item.audit_company_name + item.audit_publish_date}
                isLoading={isPlaceholderData}
              >
                {`${item.audit_company_name}, ${dayjs(
                  item.audit_publish_date
                ).format("MMM DD, YYYY")}`}
              </LinkExternal>
            ))}
          </ContainerWithScrollY>
