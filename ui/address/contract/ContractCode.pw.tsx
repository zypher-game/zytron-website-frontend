...
const CONTRACT_AUDITS_API_URL = buildApiUrl("contract_security_audits", {
  hash: addressHash,
});
const hooksConfig = {
  router: {
...
test("full view +@mobile +@dark-mode", async ({ mount, page }) => {
  ...
  const PROXY_CONTRACT_API_URL = buildApiUrl("contract", {
    hash: addressMock.contract.implementation_address as string,
  });
...
test("verified with changed byte code socket", async ({
  mount,
  page,
  createSocket,
}) => {
...
test("verified via lookup in eth_bytecode_db", async ({
  mount,
  page,
  createSocket,
}) => {
...
test("self destructed", async ({ mount, page }) => {
  ...
  const section = page.locator("section", {
    hasText: "Contract creation code",
  });
  await expect(section).toHaveScreenshot();
});
...
test.describe("with audits feature", () => {
  ...
  withAuditsTest("no audits", async ({ mount, page }) => {
    ...
    await page.route(CONTRACT_AUDITS_API_URL, (route) =>
      route.fulfill({
        status: 200,
