...
type RpcResponseType = [number | null];
...
export default function useAddressQuery({
  ...
  const rpcQuery = useQuery<RpcResponseType, unknown, AddressCounters | null>({
    ...
    queryFn: async () => {
      ...
      const txCount = publicClient
        .getTransactionCount({ address: hash as `0x${string}` })
        .catch(() => null);

      return Promise.all([txCount]);
    },
    select: (response) => {
  ...
  const isRpcQuery = Boolean(
    (addressQuery.isDegradedData || apiQuery.isError) &&
      rpcQuery.data &&
      publicClient
  );
