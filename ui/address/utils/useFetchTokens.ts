...
const tokenBalanceItemIdentityFactory =
  (match: AddressTokenBalance) => (item: AddressTokenBalance) =>
...
export default function useFetchTokens({ hash }: Props) {
  ...
  const updateTokensData = React.useCallback(
    (type: TokenType, payload: AddressTokensBalancesSocketMessage) => {
      const queryKey = getResourceKey("address_tokens", {
        pathParams: { hash },
        queryParams: { type },
      });
      ...
      queryClient.setQueryData(
        queryKey,
        (prevData: AddressTokensResponse | undefined) => {
          const items =
            prevData?.items.map((currentItem) => {
              const updatedData = payload.token_balances.find(
                tokenBalanceItemIdentityFactory(currentItem)
              );
          ...
          const extraItems = prevData?.next_page_params
            ? []
            : payload.token_balances.filter(
                (socketItem) =>
                  !items.some(tokenBalanceItemIdentityFactory(socketItem))
              );
  ...
  const handleTokenBalancesErc20Message: SocketMessage.AddressTokenBalancesErc20["handler"] =
    React.useCallback(
      (payload) => {
  ...
  const handleTokenBalancesErc721Message: SocketMessage.AddressTokenBalancesErc721["handler"] =
    React.useCallback(
      (payload) => {
  ...
  const handleTokenBalancesErc1155Message: SocketMessage.AddressTokenBalancesErc1155["handler"] =
    React.useCallback(
      (payload) => {
  ...
  const handleTokenBalancesErc404Message: SocketMessage.AddressTokenBalancesErc1155["handler"] =
    React.useCallback(
      (payload) => {
