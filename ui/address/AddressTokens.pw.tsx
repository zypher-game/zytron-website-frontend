...
const API_URL_NFT =
  buildApiUrl("address_nfts", { hash: ADDRESS_HASH }) + "?type=";
const API_URL_COLLECTIONS =
  buildApiUrl("address_collections", { hash: ADDRESS_HASH }) + "?type=";
...
const test = base.extend({
  page: async ({ page }, use) => {
    ...
    await page.route(API_URL_TOKENS + "?type=ERC-404", (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify(response404),
      })
    );
    await page.route(API_URL_NFT, (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify(tokensMock.nfts),
      })
    );
    await page.route(API_URL_COLLECTIONS, (route) =>
      route.fulfill({
        status: 200,
...
base.describe("update balances via socket", () => {
  ...
  test("", async ({ mount, page, createSocket }) => {
    ...
    const channel = await socketServer.joinChannel(
      socket,
      `addresses:${ADDRESS_HASH.toLowerCase()}`
    );
