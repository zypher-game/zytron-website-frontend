...
const API_URL_COUNTERS = buildApiUrl("address_counters", {
  hash: ADDRESS_HASH,
});
const API_URL_TOKENS_ERC20 =
  buildApiUrl("address_tokens", { hash: ADDRESS_HASH }) + "?type=ERC-20";
const API_URL_TOKENS_ERC721 =
  buildApiUrl("address_tokens", { hash: ADDRESS_HASH }) + "?type=ERC-721";
const API_URL_TOKENS_ER1155 =
  buildApiUrl("address_tokens", { hash: ADDRESS_HASH }) + "?type=ERC-1155";
const API_URL_TOKENS_ERC404 =
  buildApiUrl("address_tokens", { hash: ADDRESS_HASH }) + "?type=ERC-404";
const hooksConfig = {
  router: {
...
test("token", async ({ mount, page }) => {
  ...
  const component = await mount(
    <TestApp>
      <MockAddressPage>
        <AddressDetails
          addressQuery={{ data: addressMock.token } as AddressQuery}
        />
      </MockAddressPage>
    </TestApp>,
    { hooksConfig }
