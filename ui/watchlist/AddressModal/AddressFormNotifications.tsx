...
export default function AddressFormNotifications<
  ...
  Checkboxes extends Path<Inputs>
>({ control }: Props<Inputs>) {
  ...
  return (
    <Grid
      ...
      gap={{ base: "10px 24px", lg: "20px 24px" }}
    >
      {NOTIFICATIONS.map((notification: string, index: number) => {
        const incomingFieldName =
          `notification_settings.${notification}.incoming` as Checkboxes;
        const outgoingFieldName =
          `notification_settings.${notification}.outcoming` as Checkboxes;
        return (
