...
const AddressForm: React.FC<Props> = ({
  ...
  if (!data?.notification_settings) {
    NOTIFICATIONS.forEach(
      (n) => (notificationsDefault[n] = { incoming: true, outcoming: true })
    );
  } else {
  ...
  const {
    ...
    defaultValues: {
      ...
      tag: data?.name || "",
      notification: data?.notification_methods
  ...
  const { mutate } = useMutation({
    ...
    onError: (error: ResourceErrorAccount<WatchlistErrors>) => {
      ...
      if (errorMap?.address_hash || errorMap?.name) {
        ...
        setError("address", {
          type: "custom",
          message: getErrorMessage(errorMap, "watchlist_id"),
        });
      } else {
