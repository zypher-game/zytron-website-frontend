...
const WatchListAddressItem = ({
  ...
  const { usdBn: usdNative } = getCurrencyValue({
  ...
  return (
    <VStack spacing={3} align="stretch" fontWeight={500}>
      ...
      <Flex fontSize="sm" pl={7} flexWrap="wrap" alignItems="center" rowGap={1}>
        <TokenEntity.Icon token={nativeTokenData} isLoading={isLoading} />
      ...
      {item.tokens_count && (
        <HStack spacing={2} fontSize="sm" pl={7}>
          ...
          <Skeleton isLoaded={!isLoading} display="inline-flex">
            ...
            <Text variant="secondary" fontWeight={400}>{`${nbsp}($${BigNumber(
        ...
        <HStack spacing={2} fontSize="sm" pl={7}>
          ...
          <Skeleton isLoaded={!isLoading} display="inline-flex">
            <Text>
              {`Net worth:${nbsp}`}
              {`${item.tokens_overflow ? ">" : ""}
                $${BigNumber(item.tokens_fiat_value)
                  .plus(BigNumber(item.address_balance ? usdNative : "0"))
                  .toFormat(2)}`}
            </Text>
          </Skeleton>
        </HStack>
