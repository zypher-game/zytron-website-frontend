...
const BlobData = ({ data, isLoading, hash }: Props) => {
  ...
  const formats = isImage
    ? FORMATS
    : FORMATS.filter((format) => format !== "Image");
  ...
  const handleDownloadButtonClick = React.useCallback(() => {
    const fileBlob = (() => {
      switch (format) {
        ...
        case "UTF-8": {
          return new Blob([hexToUtf8(data)], {
            type: guessedType?.mime ?? "text/plain",
          });
        }
        case "Base64": {
          return new Blob([hexToBase64(data)], {
      ...
      case "UTF-8":
        return (
          <RawDataSnippet
            ...
            showCopy={false}
            isLoading={isLoading}
            contentProps={{ wordBreak: "break-word" }}
          />
        );
      case "Base64":
        return (
