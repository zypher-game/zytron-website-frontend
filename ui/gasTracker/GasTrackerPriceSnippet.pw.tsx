...
test("with usd as primary unit +@dark-mode", async ({ mount, page }) => {
  await mount(
    <TestApp>
      <GasTrackerPriceSnippet data={data} type="fast" isLoading={false} />
    </TestApp>
  );
...
test("loading state", async ({ mount, page }) => {
  await mount(
    <TestApp>
      <GasTrackerPriceSnippet data={data} type="fast" isLoading={true} />
    </TestApp>
  );
  await expect(page).toHaveScreenshot({ clip });
});
...
gweiUnitsTest(
  ...
  async ({ mount, page }) => {
    await mount(
      <TestApp>
        <GasTrackerPriceSnippet data={data} type="slow" isLoading={false} />
      </TestApp>
    );
    await expect(page).toHaveScreenshot({ clip });
  }
