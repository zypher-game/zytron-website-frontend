...
type StateData = AddressVerificationFormFirstStepFields &
  AddressCheckStatusSuccess & { isToken?: boolean };
...
const AddressVerificationModal = ({
  ...
  React.useEffect(() => {
    isOpen &&
      mixpanel.logEvent(mixpanel.EventTypes.VERIFY_ADDRESS, {
        Action: "Form opened",
        "Page type": pageType,
      });
  }, [isOpen, pageType]);
  ...
  const handleGoToSecondStep = React.useCallback(
    (firstStepResult: typeof data) => {
      setData(firstStepResult);
      setStepIndex((prev) => prev + 1);
      mixpanel.logEvent(mixpanel.EventTypes.VERIFY_ADDRESS, {
        Action: "Address entered",
        "Page type": pageType,
      });
    },
    [pageType]
  ...
  const handleGoToThirdStep = React.useCallback(
    (address: VerifiedAddress, signMethod: "wallet" | "manual") => {
      onSubmit(address);
      setStepIndex((prev) => prev + 1);
      setData((prev) => ({
        ...prev,
        isToken: Boolean(address.metadata.tokenName),
      }));
      mixpanel.logEvent(mixpanel.EventTypes.VERIFY_ADDRESS, {
  ...
  const steps = [
    {
      ...
      content: (
        <AddressVerificationStepAddress
          onContinue={handleGoToSecondStep}
