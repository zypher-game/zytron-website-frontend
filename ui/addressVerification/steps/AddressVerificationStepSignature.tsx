...
interface Props
  extends AddressVerificationFormFirstStepFields,
    AddressCheckStatusSuccess {
...
const AddressVerificationStepSignature = ({
  ...
  const [signMethod, setSignMethod] = React.useState<SignMethod>(
    noWeb3Provider ? "manual" : "wallet"
  );
      ...
      try {
        ...
        if (response.status !== "SUCCESS") {
          ...
          return setError("root", {
            type,
            message:
              response.status === "INVALID_SIGNER_ERROR"
  ...
  const handleSignMethodChange = React.useCallback(
    (value: typeof signMethod) => {
      setSignMethod(value);
      clearErrors("root");
    },
    [clearErrors]
  ...
  const handleWeb3SignClick = React.useCallback(() => {
    ...
    if (!isConnected) {
      return setError("root", {
        type: "manual",
        message: "Please connect to your Web3 wallet first",
      });
    }
  ...
  const rootError = (() => {
    switch (formState.errors.root?.type) {
      ...
      case "VALIDITY_EXPIRED_ERROR": {
        return (
          <span>
            This verification message has expired. Add the contract address to
            restart the process.
          </span>
        );
      }
      ...
      case "INVALID_SIGNER_ERROR": {
        ...
        const expectedSigners = [contractCreator, contractOwner]
          .filter(Boolean)
          .map((s) => shortenString(s))
          .join(", ");
        return (
          <Box>
            ...
            <span>
              {" "}
              is not a creator/owner of the requested contract and cannot claim
              ownership. Only{" "}
            </span>
      ...
      case "UNKNOWN_STATUS": {
        return (
          <Box>
            <span>
              We are not able to process the verify account ownership for this
              contract address. Kindly{" "}
            </span>
            {contactUsLink}
  ...
  return (
    <form noValidate onSubmit={onSubmit}>
      ...
      <Box mb={8}>
        <span>
          Please select the address to sign and copy the message and sign it
          using the Blockscout message provider of your choice.{" "}
        </span>
        <Link
          href="https://docs.blockscout.com/for-users/my-account/verified-addresses/copy-and-sign-message"
          target="_blank"
        >
          Additional instructions
        </Link>
        <span>
          . If you do not see your address here but are sure that you are the
          owner of the contract, kindly{" "}
        </span>
        {contactUsLink}
        <span> for further assistance.</span>
      </Box>
