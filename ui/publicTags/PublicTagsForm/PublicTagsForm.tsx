...
const PublicTagsForm = ({ changeToDataScreen, data }: Props) => {
    ...
    defaultValues: {
      ...
      tags:
        data?.tags
          ...
          .map((tag) => tag)
          .join("; ") || "",
      addresses: data?.addresses?.map((address, index: number) => ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      queryClient.setQueryData(
        [resourceKey("public_tags")],
        (prevData: PublicTags | undefined) => {
          const isExisting =
            prevData && prevData.some((item) => item.id === response.id);
    ...
    onError: (error: ResourceErrorAccount<PublicTagErrors>) => {
      ...
      if (
        ...
        errorMap?.tags ||
        errorMap?.addresses ||
        errorMap?.additional_comment
      ) {
        errorMap?.full_name &&
          setError("fullName", {
            type: "custom",
            message: getErrorMessage(errorMap, "full_name"),
          });
        errorMap?.email &&
          setError("email", {
            type: "custom",
            message: getErrorMessage(errorMap, "email"),
          });
        errorMap?.tags &&
          setError("tags", {
            type: "custom",
            message: getErrorMessage(errorMap, "tags"),
          });
        errorMap?.addresses &&
          setError("addresses.0.address", {
            type: "custom",
            message: getErrorMessage(errorMap, "addresses"),
          });
        errorMap?.additional_comment &&
          setError("comment", {
            type: "custom",
            message: getErrorMessage(errorMap, "additional_comment"),
          });
      } else {
  ...
  return (
    <chakra.form
      ...
      width={{
        base: "auto",
        lg: `calc(100% - ${ADDRESS_INPUT_BUTTONS_WIDTH}px)`,
      }}
      maxWidth="844px"
