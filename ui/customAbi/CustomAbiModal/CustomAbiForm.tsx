...
const CustomAbiForm: React.FC<Props> = ({ data, onClose, setAlertVisible }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(
        [resourceKey("custom_abi")],
        (prevData: CustomAbis | undefined) => {
          const isExisting =
    ...
    onError: (error: ResourceErrorAccount<CustomAbiErrors>) => {
      ...
      if (errorMap?.address_hash || errorMap?.name || errorMap?.abi) {
        errorMap?.address_hash &&
          setError("contract_address_hash", {
            type: "custom",
            message: getErrorMessage(errorMap, "address_hash"),
          });
        errorMap?.name &&
          setError("name", {
            type: "custom",
            message: getErrorMessage(errorMap, "name"),
          });
        errorMap?.abi &&
          setError("abi", {
            type: "custom",
            message: getErrorMessage(errorMap, "abi"),
          });
      } else if (errorMap?.identity_id) {
        setError("contract_address_hash", {
          type: "custom",
          message: getErrorMessage(errorMap, "identity_id"),
        });
      } else {
  ...
  const renderContractAddressInput = useCallback(
    ({
      ...
      field: ControllerRenderProps<Inputs, "contract_address_hash">;
    }) => {
      return (
