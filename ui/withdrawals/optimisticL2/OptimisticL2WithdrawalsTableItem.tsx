...
const OptimisticL2WithdrawalsTableItem = ({ item, isLoading }: Props) => {
  const timeAgo = item.l2_timestamp
    ? dayjs(item.l2_timestamp).fromNow()
    : "N/A";
  const timeToEnd = item.challenge_period_end
    ? dayjs(item.challenge_period_end).fromNow(true) + " left"
    : "";
    ...
    <Tr>
      ...
      <Td verticalAlign="middle">
        {item.status === "Ready for relay" && rollupFeature.L2WithdrawalUrl ? (
          <LinkExternal href={rollupFeature.L2WithdrawalUrl}>
            {item.status}
          </LinkExternal>
        ) : (
          <Skeleton isLoaded={!isLoading} display="inline-block">
