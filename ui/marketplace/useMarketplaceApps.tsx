...
function isAppCategoryMatches(
  ...
  return (
    ...
    (category === MarketplaceCategory.FAVORITES &&
...
export default function useMarketplaceApps(
  ...
  const [snapshotFavoriteApps, setSnapshotFavoriteApps] = React.useState<
  ...
  const { isPlaceholderData, isError, error, data } = useQuery<
    unknown,
    ResourceError<unknown>,
    Array<MarketplaceAppWithSecurityReport>
  >({
    ...
    queryFn: async () => {
      if (!feature.isEnabled) {
        ...
        return fetch<Array<MarketplaceAppWithSecurityReport>, unknown>(
          feature.configUrl,
          undefined,
          { resource: "marketplace-dapps" }
        );
      } else {
        return apiFetch("marketplace_dapps", {
    ...
    select: (data) =>
      sortApps(
        data as Array<MarketplaceAppWithSecurityReport>,
        snapshotFavoriteApps || []
      ),
    placeholderData: feature.isEnabled
      ? Array(9).fill(MARKETPLACE_APP)
      : undefined,
    staleTime: Infinity,
    enabled:
      feature.isEnabled && (!favoriteApps || Boolean(snapshotFavoriteApps)),
  });
  ...
  const displayedApps = React.useMemo(() => {
    return (
      appsWithSecurityReports?.filter(
        (app) =>
