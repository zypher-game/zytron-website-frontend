...
const AppSecurityReport = ({
  ...
  const handleLinkClick = React.useCallback(() => {
    mixpanel.logEvent(mixpanel.EventTypes.PAGE_WIDGET, {
      Type: "Analyzed contracts",
      Info: id,
      Source: "Security score popup",
    });
    showContractList();
  }, [id, showContractList]);
    ...
    <Popover isOpen={isOpen} onClose={onClose} placement="bottom-start" isLazy>
      ...
      <PopoverContent w={{ base: "100vw", lg: "328px" }}>
        <PopoverBody px="26px" py="20px" fontSize="sm">
          <Box mb={5}>
            {solidityScanContractsNumber} smart contract
            {solidityScanContractsNumber === 1 ? " was" : "s were"} evaluated to
            determine this protocol{apos}s overall security score on the{" "}
            {config.chain.name} network.
          </Box>
          <SolidityscanReportScore score={securityScore} mb={5} />
          {issueSeverityDistribution && totalIssues > 0 && (
            <Box mb={5}>
              <Text py="7px" variant="secondary" fontSize="xs" fontWeight={500}>
                Threat score & vulnerabilities
              </Text>
              <SolidityscanReportDetails
                vulnerabilities={issueSeverityDistribution}
