...
const EmptySearchResult = ({ favoriteApps, selectedCategoryId }: Props) => (
  <EmptySearchResultDefault
    text={
      selectedCategoryId === MarketplaceCategory.FAVORITES &&
      !favoriteApps.length ? (
        <>
          You don{apos}t have any favorite apps. Click on the{" "}
          <IconSvg name="star_outline" w={4} h={4} mb={-0.5} /> icon on the app
          {apos}s card to add it to Favorites.
        </>
      ) : (
        <>
          ...
          {"suggestIdeasFormUrl" in feature && (
            <>
              ...
              <LinkExternal href={feature.suggestIdeasFormUrl}>
                Share it with us
              </LinkExternal>
            </>
          )}
