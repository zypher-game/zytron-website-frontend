...
const securityReportsUrl =
  (feature.isEnabled && feature.securityReportsUrl) || "";
...
export default function useSecurityReports() {
  ...
  return useQuery<
    ...
    Record<string, MarketplaceAppSecurityReport>
  >({
    queryKey: ["marketplace-security-reports"],
    queryFn: async () =>
      apiFetch(securityReportsUrl, undefined, {
        resource: "marketplace-security-reports",
      }),
    select: (data) => {
      ...
      (data as Array<MarketplaceAppSecurityReportRaw>).forEach((item) => {
        ...
        if (report) {
          const issues: Record<string, number> =
            report.overallInfo.issueSeverityDistribution;
          report.overallInfo.totalIssues = Object.values(issues).reduce(
            (acc, val) => acc + val,
            0
          );
          report.overallInfo.securityScore = Number(
            report.overallInfo.securityScore.toFixed(2)
          );
        }
