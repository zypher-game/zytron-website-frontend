...
function getFavoriteApps() {
  try {
    return JSON.parse(
      localStorage.getItem(favoriteAppsLocalStorageKey) || "[]"
    ) as Array<string>;
  } catch (e) {
    return [];
  }
...
export default function useMarketplace() {
  ...
  const [selectedDisplayType, setSelectedDisplayType] = React.useState<string>(
    Object.values(MarketplaceDisplayType).includes(
      defaultDisplayType as MarketplaceDisplayType
    )
      ? defaultDisplayType
  ...
  const [isFavoriteAppsLoaded, setIsFavoriteAppsLoaded] =
    React.useState<boolean>(false);
  const [isAppInfoModalOpen, setIsAppInfoModalOpen] =
    React.useState<boolean>(false);
  const [isDisclaimerModalOpen, setIsDisclaimerModalOpen] =
    React.useState<boolean>(false);
  const [contractListModalType, setContractListModalType] =
    React.useState<ContractListTypes | null>(null);
  ...
  const handleFavoriteClick = React.useCallback(
    (
      id: string,
      isFavorite: boolean,
      source: "Discovery view" | "Security view" | "App modal"
    ) => {
  ...
  const showContractList = React.useCallback(
    (id: string, type: ContractListTypes, hasPreviousStep?: boolean) => {
      setSelectedAppId(id);
  ...
  const handleCategoryChange = React.useCallback((newCategory: string) => {
    mixpanel.logEvent(mixpanel.EventTypes.FILTERS, {
      Source: "Marketplace",
      "Filter name": newCategory,
    });
    setSelectedCategoryId(newCategory);
  }, []);

  const handleDisplayTypeChange = React.useCallback(
    (newDisplayType: MarketplaceDisplayType) => {
      setSelectedDisplayType(newDisplayType);
    },
    []
  );

  const { isPlaceholderData, isError, error, data, displayedApps } =
    useMarketplaceApps(
  ...
  const { isPlaceholderData: isCategoriesPlaceholderData, data: categories } =
  ...
  React.useEffect(() => {
    if (!isPlaceholderData && !isError) {
      const isValidDefaultCategory = categories
        .map((c) => c.name)
        .includes(defaultCategoryId);
  ...
  React.useEffect(() => {
    const query = _pickBy(
      {
        category:
          selectedCategoryId === MarketplaceCategory.ALL
            ? undefined
            : selectedCategoryId,
        filter: debouncedFilterQuery,
        tab:
          selectedDisplayType === MarketplaceDisplayType.DEFAULT
            ? undefined
            : selectedDisplayType,
      },
      Boolean
    ...
    if (debouncedFilterQuery.length > 0) {
      mixpanel.logEvent(mixpanel.EventTypes.LOCAL_SEARCH, {
        Source: "Marketplace",
        "Search query": debouncedFilterQuery,
      });
    }
