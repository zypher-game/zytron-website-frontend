...
const ApiKeyForm: React.FC<Props> = ({ data, onClose, setAlertVisible }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      queryClient.setQueryData(
        [resourceKey("api_keys")],
        (prevData: ApiKeys | undefined) => {
          const isExisting =
            prevData &&
            prevData.some((item) => item.api_key === response.api_key);
  ...
  const renderTokenInput = useCallback(
    ({ field }: { field: ControllerRenderProps<Inputs, "token"> }) => {
      return (
        <FormControl variant="floating" id="address">
          <Input {...field} isDisabled={true} />
          <FormLabel data-in-modal="true">
    ...
    ({ field }: { field: ControllerRenderProps<Inputs, "name"> }) => {
      return (
        ...
        >
          ...
          />
          <InputPlaceholder
            text="Application name for API key (e.g Web3 project)"
            error={errors.name}
          />
        </FormControl>
      );
