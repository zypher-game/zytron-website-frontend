...
const TokenInfoForm = ({
  ...
  React.useEffect(() => {
    if (!application?.id && !openEventSent.current) {
      mixpanel.logEvent(mixpanel.EventTypes.VERIFY_TOKEN, {
        Action: "Form opened",
      });
    ...
    async (data) => {
      try {
        ...
        const isNewApplication =
          !application?.id ||
          ["REJECTED", "APPROVED"].includes(application.status);
        ...
        >("token_info_applications", {
          pathParams: {
            ...
            id: !isNewApplication ? application.id : undefined,
          },
          fetchParams: {
        ...
        toast({
          ...
          title: "Error",
          description:
            (error as ResourceError<{ message: string }>)?.payload?.message ||
    ...
    >
      ...
      >
        ...
        <TokenInfoFieldProjectSector
          {...fieldProps}
          config={configQuery.data.projectSectors}
        />
        ...
        <TokenInfoFieldPriceTicker
          {...fieldProps}
          name="ticker_coin_market_cap"
          label="CoinMarketCap URL"
        />
        <TokenInfoFieldPriceTicker
          {...fieldProps}
          name="ticker_coin_gecko"
          label="CoinGecko URL"
        />
        <GridItem colSpan={{ base: 1, lg: 2 }}>
          <TokenInfoFieldPriceTicker
            {...fieldProps}
            name="ticker_defi_llama"
            label="DefiLlama URL "
          />
        </GridItem>
