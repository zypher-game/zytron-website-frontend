    ...
    <ListItemMobile display="block" width="100%" isAnimated key={tx.hash}>
      <Flex justifyContent="space-between" mt={4}>
        <HStack flexWrap="wrap">
          {tx.translation ?
            <TxTranslationType types={tx.tx_types ?? tx.transaction_types ?? []} isLoading={isLoading || tx.translation.isLoading} translatationType={tx.translation.data?.type} /> :
            <TxType types={tx.tx_types ?? tx.transaction_types ?? []} isLoading={isLoading} />
          }
      ...
      <Flex justifyContent="space-between" lineHeight="24px" mt={3} alignItems="center">
        <TxEntity
          ...
          fontWeight="700"
          iconName={(tx.tx_types ?? tx.transaction_types ?? []).includes('blob_transaction') ? 'blob' : undefined}
        />
        {tx.timestamp && (
