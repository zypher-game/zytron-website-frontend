...
export const createAddressValues = (
  translateData: NovesResponseData,
  description: string
) => {
  const addressMatches = description.match(ADDRESS_REGEXP);
  const contractMatches = description.match(CONTRACT_REGEXP);

  let descriptionAddresses: Array<string> = addressMatches
    ? addressMatches
    : [];
  let contractAddresses: Array<string> = [];

  if (contractMatches?.length) {
    contractAddresses = contractMatches.map(
      (text) => text.split(ADDRESS_REGEXP)[1]
    );
    descriptionAddresses =
      addressMatches?.filter(
  ...
  const descriptionSummaryValues = createAddressSummaryValues(
    descriptionAddresses,
    addresses
  );
...
const createAddressSummaryValues = (
  descriptionAddresses: Array<string>,
  addresses: Array<SummaryAddress>,
  isContract = false
) => {
  const summaryValues: Array<SummaryValues | undefined> =
    descriptionAddresses.map((match) => {
      const address = addresses.find((address) =>
        address.hash.toUpperCase().startsWith(match.toUpperCase())
      );
  ...
  return summaryValues.filter(
    (value) => value !== undefined
  ) as Array<SummaryValues>;
};

function extractAddresses(data: NovesResponseData) {
  const addressesSet: Set<{ hash: string | null; name?: string | null }> =
    new Set(); // Use a Set to store unique addresses
  ...
  if (data.txTypeVersion === 2) {
    data.classificationData.sent.forEach((transaction) => {
      addressesSet.add({
        hash: transaction.from.address,
        name: transaction.from.name,
      });
      addressesSet.add({
        hash: transaction.to.address,
  ...
  const addresses = Array.from(addressesSet) as Array<{
    hash: string;
    name?: string;
  ...
  return addresses.filter(
    (address) => address.hash !== null && address.hash !== ""
  );
}
