...
const UserOpDetails = ({ query }: Props) => {
  ...
  return (
    <Grid
      ...
      templateColumns={{
        base: "minmax(0, 1fr)",
        lg: "minmax(min-content, 220px) minmax(0, 1fr)",
      }}
      overflow="hidden"
    >
      <DetailsInfoItem
      ...
      >
        ...
        <Utilization
          ml={4}
          colorScheme="gray"
          value={BigNumber(data.gas_used)
            .dividedBy(BigNumber(data.gas))
            .toNumber()}
          isLoading={isPlaceholderData}
        />
      </DetailsInfoItem>
      ...
      {config.features.txInterpretation.isEnabled && (
        <UserOpDetailsActions
          hash={data.hash}
          isUserOpDataLoading={isPlaceholderData}
        />
      )}
        ...
        <>
          ...
          {!config.UI.views.tx.hiddenFields?.gas_fees && (
            <>
              ...
              >
                <Text>
                  {BigNumber(data.max_fee_per_gas).dividedBy(WEI).toFixed()}{" "}
                  {currencyUnits.ether}{" "}
                </Text>
