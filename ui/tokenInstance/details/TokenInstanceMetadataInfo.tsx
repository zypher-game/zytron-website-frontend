...
const TokenInstanceMetadataInfo = ({ data, isLoading }: Props) => {
  ...
  const hasMetadata =
    metadata &&
    Boolean(metadata.name || metadata.description || metadata.attributes);
    ...
    <>
      ...
      {metadata?.description && (
        <DetailsInfoItem
          ...
          isLoading={isLoading}
        >
          <Skeleton isLoaded={!isLoading}>{metadata.description}</Skeleton>
        </DetailsInfoItem>
      )}
      {metadata?.attributes && (
        <DetailsInfoItem
        ...
        >
          ...
          >
            {metadata.attributes.map((attribute, index) => (
              <Item key={index} data={attribute} isLoading={isLoading} />
            ))}
          </Grid>
        </DetailsInfoItem>
      )}
