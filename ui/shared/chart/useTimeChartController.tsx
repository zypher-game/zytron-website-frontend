...
export default function useTimeChartController({
  ...
  const axesParams = React.useMemo(
    () => getAxesParams(data, axesConfig),
    [data, axesConfig, rect?.width]
  );

  const chartMargin = React.useMemo(() => {
    const PIXELS_PER_DIGIT = 8;
    const leftShift = axesConfig?.y?.noLabel
      ? 0
      : PIXELS_PER_DIGIT * axesParams.y.labelFormatParams.maxLabelLength;
    ...
    axesParams.y.labelFormatParams.maxLabelLength,
    margin,
    axesConfig?.y?.noLabel,
  ]);
