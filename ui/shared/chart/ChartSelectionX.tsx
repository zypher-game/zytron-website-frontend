...
const ChartSelectionX = ({
  ...
  const getIndexByX = React.useCallback(
    (x: number) => {
      ...
      const bisectDate = d3.bisector<TimeChartItem, unknown>(
        (d) => d.date
      ).left;
  ...
  const handleSelect = React.useCallback(
    (x0: number, x1: number) => {
      ...
      if (
        Math.abs(dayjs(startDate).diff(endDate, "day")) > SELECTION_THRESHOLD
      ) {
        onSelect([
          dayjs.min(dayjs(startDate), dayjs(endDate)).toDate(),
          dayjs.max(dayjs(startDate), dayjs(endDate)).toDate(),
        ]);
      }
    },
    [onSelect, scale]
