...
const ChartLineLoader = ({ className }: { className?: string }) => {
  ...
  const lineBgColor = useToken(
    "colors",
    useColorModeValue("gray.200", "gray.500")
  );
  const lineFgColor = useToken(
    "colors",
  ...
  React.useEffect(() => {
    ...
    const animatePath = () => {
      ...
      const nextOffset =
        offset.current + INCREMENT <= length ? offset.current + INCREMENT : 0;
