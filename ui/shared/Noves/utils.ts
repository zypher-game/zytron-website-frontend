...
export const getFromTo = (
  ...
  if (sent && sent[0]) {
    sentFound = sent
      .filter(
        (sent) => sent.from.address.toLocaleLowerCase() === currentAddress
      )
  ...
  if (received && received[0]) {
    receivedFound = received
      .filter(
        (received) =>
          received.to.address?.toLocaleLowerCase() === currentAddress
      )
  ...
  if (
    raw.fromAddress &&
    raw.fromAddress.toLocaleLowerCase() === currentAddress
  ) {
    return { text: "Sent to", address: raw.toAddress };
  }
