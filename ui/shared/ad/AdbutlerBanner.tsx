...
import {
  ...
  placeAd,
  ADBUTLER_ACCOUNT,
} from "ui/shared/ad/adbutlerScript";
...
const AdbutlerBanner = ({ className }: { className?: string }) => {
  ...
  React.useEffect(() => {
    ...
    if (isBrowser() && window.AdButler) {
      ...
      const adButlerConfig = isMobile
        ? feature.adButler.config.mobile
        : feature.adButler.config.desktop;
  ...
  return (
    <Flex className={className} id="adBanner" h={{ base: "100px", lg: "90px" }}>
      <Script strategy="lazyOnload" id="ad-butler-1">
        {connectAdbutler}
      </Script>
      <Script strategy="lazyOnload" id="ad-butler-2">
        {placeAd}
      </Script>
