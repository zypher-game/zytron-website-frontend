...
export default function useAdaptiveTabs(
  ...
  const [tabsRefs, setTabsRefs] = React.useState<
    Array<React.RefObject<HTMLButtonElement>>
  >([]);
  ...
  const calculateCut = React.useCallback(() => {
    ...
    const rightSlotWidth =
      rightSlotRef.current?.getBoundingClientRect().width || 0;
    const tabWidths = tabsRefs.map(
      (tab) => tab.current?.getBoundingClientRect().width
    );
    ...
    const { visibleNum } = tabWidths.slice(0, -1).reduce(
      (result, item, index, array) => {
        ...
        if (index === array.length - 1) {
          ...
          if (result.accWidth + item < listWidth - rightSlotWidth) {
            return {
              visibleNum: result.visibleNum + 1,
              accWidth: result.accWidth + item,
            };
          }
        } else {
