  ...
  return (
    <>
      ...
      <Flex justifyContent="space-between" mr={3}>
        <Text>{item.name}</Text>
        <Text
          color={vulnerabilities[item.id] > 0 ? "text" : yetAnotherGrayColor}
        >
      ...
      <Box bg={bgBar} h="10px" borderRadius="8px">
        <Box
          bg={item.color}
          w={vulnerabilities[item.id] / vulnerabilitiesCount}
          h="10px"
          borderRadius="8px"
        />
      </Box>
...
const SolidityscanReportDetails = ({
  vulnerabilities,
  vulnerabilitiesCount,
}: Props) => {
  return (
