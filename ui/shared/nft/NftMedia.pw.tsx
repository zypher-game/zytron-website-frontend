...
test.describe("no url", () => {
  ...
  test("non-media url and fallback", async ({ mount, page }) => {
    ...
    const ANIMATION_MEDIA_TYPE_API_URL = `/node-api/media-type?url=${encodeURIComponent(
      ANIMATION_URL
    )}`;
    const IMAGE_URL = "https://localhost:3000/my-image.jpg";
...
test.describe("page", () => {
  ...
  const MEDIA_TYPE_API_URL = `/node-api/media-type?url=${encodeURIComponent(
    MEDIA_URL
  )}`;
