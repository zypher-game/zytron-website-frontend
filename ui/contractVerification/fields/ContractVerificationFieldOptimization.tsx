...
const ContractVerificationFieldOptimization = () => {
  ...
  const error =
    "optimization_runs" in formState.errors
      ? formState.errors.optimization_runs
      : undefined;
  ...
  const renderCheckboxControl = React.useCallback(
    ({
      field,
    }: {
      field: ControllerRenderProps<FormFields, "is_optimization_enabled">;
    }) => (
      <Flex flexShrink={0}>
  ...
  const renderInputControl = React.useCallback(
    ({
      ...
      field: ControllerRenderProps<FormFields, "optimization_runs">;
    }) => {
      return (
        <Input
          {...field}
