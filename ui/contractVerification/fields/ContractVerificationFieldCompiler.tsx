...
const ContractVerificationFieldCompiler = ({ isVyper }: Props) => {
  ...
  const { formState, control, getValues, resetField } =
    useFormContext<FormFields>();
  const isMobile = useIsMobile();
  const queryClient = useQueryClient();
  const config = queryClient.getQueryData<SmartContractVerificationConfig>(
    getResourceKey("contract_verification_config")
  );

  const handleCheckboxChange = React.useCallback(() => {
    if (isNightly) {
      const field = getValues("compiler");
      field?.value.includes("nightly") &&
  ...
  const renderControl = React.useCallback(
    ({ field }: { field: ControllerRenderProps<FormFields, "compiler"> }) => {
      const error =
        "compiler" in formState.errors ? formState.errors.compiler : undefined;
    ...
    <ContractVerificationFormRow>
      ...
      {isVyper ? null : (
        <chakra.div mt={{ base: 0, lg: 8 }}>
          ...
          <span>
            . Use the compiler version rather than the nightly build. If using
            the Solidity compiler, run{" "}
          </span>
