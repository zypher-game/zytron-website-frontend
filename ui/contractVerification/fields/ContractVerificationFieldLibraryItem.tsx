...
const ContractVerificationFieldLibraryItem = ({
  control,
  index,
  fieldsLength,
  onAddFieldClick,
  ...
  const renderNameControl = React.useCallback(
    ({
      field,
    }: {
      field: ControllerRenderProps<FormFields, `libraries.${number}.name`>;
    }) => {
      return (
        <FormControl
          variant="floating"
  ...
  const renderAddressControl = React.useCallback(
    ({
      field,
    }: {
      field: ControllerRenderProps<FormFields, `libraries.${number}.address`>;
    }) => {
      return (
        <FormControl
          variant="floating"
  ...
  return (
    <>
      ...
      <ContractVerificationFormRow>
        ...
        {index === 0 ? (
          <>
            A library name called in the .sol file. Multiple libraries (up to
            10) may be added for each contract.
          </>
        ) : null}
      </ContractVerificationFormRow>
      <ContractVerificationFormRow>
        ...
        {index === 0 ? (
          <>
            The 0x library address. This can be found in the generated json file
            or Truffle output (if using truffle).
          </>
        ) : null}
      </ContractVerificationFormRow>
    </>
