...
const options = CONTRACT_LICENSES.map(({ label, title, type }) => ({
  label: `${title} (${label})`,
  value: type,
}));
...
const ContractVerificationFieldLicenseType = () => {
  ...
  const renderControl = React.useCallback(
    ({
      ...
      field: ControllerRenderProps<FormFields, "license_type">;
    }) => {
      const error =
        "license_type" in formState.errors
          ? formState.errors.license_type
          : undefined;
