...
const ContractVerificationFieldEvmVersion = ({ isVyper }: Props) => {
  ...
  const renderControl = React.useCallback(
    ({
      ...
      field: ControllerRenderProps<FormFields, "evm_version">;
    }) => {
      const error =
        "evm_version" in formState.errors
          ? formState.errors.evm_version
          : undefined;
  ...
  return (
    <ContractVerificationFormRow>
      ...
      <>
        <span>
          The EVM version the contract is written for. If the bytecode does not
          match the version, we try to verify using the latest EVM version.{" "}
        </span>
        <Link
          href={
            isVyper
