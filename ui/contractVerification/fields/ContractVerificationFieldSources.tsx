...
const ContractVerificationFieldSources = ({
  ...
  const { setValue, getValues, control, formState, clearErrors } =
  ...
  const renderUploadButton = React.useCallback(() => {
    return (
      <div>
        <Text fontWeight={500} color="text_secondary" mb={3}>
          {title}
        </Text>
  ...
  const renderControl = React.useCallback(
    ({ field }: { field: ControllerRenderProps<FormFields, typeof name> }) => {
      ...
      return (
        <>
          <FileInput<FormFields, typeof name>
  ...
  const validateFileType = React.useCallback(
    async (
      value: FieldPathValue<FormFields, typeof name>
    ): Promise<ValidateResult> => {
      if (Array.isArray(value)) {
        const errorText = `Wrong file type. Allowed files types are ${fileTypes.join(
          ","
        )}.`;
        const errors = value.map(({ name }) =>
          fileTypes.some((ext) => name.endsWith(ext)) ? "" : errorText
        );
  ...
  const validateFileSize = React.useCallback(
    async (
      ...
      if (Array.isArray(value)) {
        ...
        const errors = value.map(({ size }) =>
          size > FILE_SIZE_LIMIT
            ? "File is too big. Maximum size is 20 Mb."
            : ""
        );
        if (errors.some((item) => item !== "")) {
          return errors.join(";");
  ...
  const validateQuantity = React.useCallback(
    async (
      value: FieldPathValue<FormFields, typeof name>
    ): Promise<ValidateResult> => {
