...
const PrivateTransactionTags = () => {
  const {
    ...
    options: {
      ...
      placeholderData: {
        items: Array(3).fill(PRIVATE_TAG_TX),
  ...
  const [transactionModalData, setTransactionModalData] =
    useState<TransactionTag>();
  ...
  const description = (
    <AccountPageDescription>
      ...
      Private tags are saved in your account and are only visible when you are
      logged in.
    </AccountPageDescription>
  );
  ...
  return (
    <>
      ...
      <Skeleton mt={8} isLoaded={!isPlaceholderData} display="inline-block">
        <Button size="lg" onClick={transactionModalProps.onOpen}>
          Add transaction tag
        </Button>
      </Skeleton>
      <TransactionModal
