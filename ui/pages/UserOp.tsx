...
const UserOp = () => {
  ...
  const txQuery = useTxQuery({
    hash: userOpQuery.data?.transaction_hash,
    isEnabled: !userOpQuery.isPlaceholderData,
  });
      ...
      if (!userOpQuery.data) {
        ...
        if (
          inRange(
            ...
            userOpQuery.data?.user_logs_start_index +
              userOpQuery.data?.user_logs_count
          )
        ) {
          return true;
        }
    ...
    () => [
      {
        id: "index",
        title: "Details",
        component: <UserOpDetails query={userOpQuery} />,
      },
      {
        id: "token_transfers",
        ...
        component: (
          <TxTokenTransfer
            txQuery={txQuery}
            tokenTransferFilter={filterTokenTransfersByLogIndex}
          />
        ),
      },
      {
        ...
        title: "Logs",
        component: (
          <TxLogs txQuery={txQuery} logsFilter={filterLogsByLogIndex} />
        ),
      },
      {
        id: "raw",
        ...
        component: (
          <UserOpRaw
            rawData={userOpQuery.data?.raw}
            isLoading={userOpQuery.isPlaceholderData}
          />
        ),
      },
    ],
  ...
  const backLink = React.useMemo(() => {
    const hasGoBackLink =
      appProps.referrer && appProps.referrer.includes("/ops");
