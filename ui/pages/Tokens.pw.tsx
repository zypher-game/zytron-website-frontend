...
base("base view +@mobile +@dark-mode", async ({ mount, page }) => {
  const allTokens = {
    items: [
      tokens.tokenInfoERC20a,
      tokens.tokenInfoERC20b,
      tokens.tokenInfoERC20c,
      tokens.tokenInfoERC20d,
      tokens.tokenInfoERC721a,
      tokens.tokenInfoERC721b,
      tokens.tokenInfoERC721c,
      tokens.tokenInfoERC1155a,
      tokens.tokenInfoERC1155b,
      tokens.tokenInfoERC1155WithoutName,
    ],
    next_page_params: {
      holder_count: 1,
      items_count: 1,
  ...
  await page.route(ALL_TOKENS_API_URL, (route) =>
    route.fulfill({
      status: 200,
  ...
  await page.route(FILTERED_TOKENS_API_URL, (route) =>
    route.fulfill({
      status: 200,
  ...
  await component
    .getByRole("textbox", { name: "Token name or symbol" })
    .focus();
  await component
    .getByRole("textbox", { name: "Token name or symbol" })
    .type("foo");
...
base.describe("bridged tokens", async () => {
  ...
  const bridgedTokens = {
    items: [tokens.bridgedTokenA, tokens.bridgedTokenB, tokens.bridgedTokenC],
    next_page_params: {
      holder_count: 1,
      items_count: 1,
      name: "a",
  ...
  test.beforeEach(async ({ page }) => {
    await page.route(BRIDGED_TOKENS_API_URL, (route) =>
      route.fulfill({
        status: 200,
  ...
  test("base view", async ({ mount, page }) => {
    await page.route(BRIDGED_TOKENS_API_URL + "?chain_ids=99", (route) =>
      route.fulfill({
        status: 200,
