...
const PublicTagsComponent: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<PublicTag> | undefined>(
    addressHash ? { addresses: [addressHash] } : undefined
  );
  ...
  if (screen === "data") {
    ...
    content = (
      <PublicTagsForm changeToDataScreen={changeToDataScreen} data={formData} />
    );
    header = formData
