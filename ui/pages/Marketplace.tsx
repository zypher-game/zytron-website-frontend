...
const Marketplace = () => {
  ...
  const handleAppClick = React.useCallback(
    (event: MouseEvent, id: string) => {
      const isShown = window.localStorage.getItem(
        "marketplace-disclaimer-shown"
      );
      if (!isShown) {
  ...
  throwOnResourceLoadError(
    isError && error ? { isError, error } : { isError: false, error: null }
  );
  ...
  return (
    <>
      ...
      {selectedDisplayType === MarketplaceDisplayType.SCORES &&
