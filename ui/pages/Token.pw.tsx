...
test.beforeEach(async ({ page }) => {
  ...
  await page.route(TOKEN_API_URL, (route) =>
    route.fulfill({
      status: 200,
  ...
  await page.route(TOKEN_COUNTERS_API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify(tokenCounters),
    })
  );
  await page.route(TOKEN_TRANSFERS_API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify({}),
    })
...
test("base view", async ({ mount, page, createSocket }) => {
  ...
  socketServer.sendMessage(socket, channel, "total_supply", {
    total_supply: 10 ** 20,
  });
...
test("with verified info", async ({ mount, page, createSocket }) => {
  ...
  await page.route(VERIFIED_INFO_URL, (route) =>
    route.fulfill({
      body: JSON.stringify(
