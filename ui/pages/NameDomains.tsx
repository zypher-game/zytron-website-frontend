...
const NameDomains = () => {
  ...
  const q =
    getQueryParamString(router.query.name) ||
    getQueryParamString(router.query.address);
  ...
  const [filterValue, setFilterValue] =
    React.useState<EnsDomainLookupFiltersOptions>(initialFilters);
  const [sort, setSort] = React.useState<Sort | undefined>(initialSort);

  const debouncedSearchTerm = useDebounce(searchTerm, 300);
  const isAddressSearch = React.useMemo(
    () => ADDRESS_REGEXP.test(debouncedSearchTerm),
    [debouncedSearchTerm]
  );
  const sortParams = getSortParamsFromValue<
    Sort,
    EnsLookupSorting["sort"],
    EnsLookupSorting["order"]
  >(sort);
  ...
  const addressesLookupQuery = useQueryWithPages({
    ...
    options: {
      ...
      placeholderData: generateListStub<"addresses_lookup">(ENS_DOMAIN, 50, {
  ...
  const domainsLookupQuery = useQueryWithPages({
    ...
    options: {
      enabled: !isAddressSearch,
      placeholderData: generateListStub<"domains_lookup">(ENS_DOMAIN, 50, {
        next_page_params: null,
      }),
    },
  });
  ...
  React.useEffect(() => {
    const hasInactiveFilter = filterValue.some(
      (value) => value === "with_inactive"
    );
    if (isAddressSearch) {
      setFilterValue(
        [
          ...
          hasInactiveFilter ? ("with_inactive" as const) : undefined,
        ].filter(Boolean)
      );
      ...
      setFilterValue(
        [hasInactiveFilter ? ("with_inactive" as const) : undefined].filter(
          Boolean
        )
      );
  ...
  const handleSortToggle = React.useCallback(
    (event: React.MouseEvent) => {
      ...
      const field = (event.currentTarget as HTMLDivElement).getAttribute(
        "data-field"
  ...
  const hasActiveFilters =
    Boolean(debouncedSearchTerm) || filterValue.length > 0;
