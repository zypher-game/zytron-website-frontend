...
const GAS_PRICE_CHART_API_URL =
  buildApiUrl("stats_line", { id: "averageGasPrice" }) + "?**";
...
test("base view +@dark-mode +@mobile", async ({ mount, page }) => {
  ...
  await page.route(STATS_LINES_API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify(statsLinesMock.base),
    })
  );
  await page.route(GAS_PRICE_CHART_API_URL, (route) =>
    route.fulfill({
      status: 200,
  ...
  await page.waitForFunction(() => {
    return (
      document
        .querySelector('path[data-name="chart-Averagegasprice-small"]')
