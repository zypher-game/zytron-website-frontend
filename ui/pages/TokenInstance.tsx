...
const TokenInstanceContent = () => {
  ...
  const backLink = React.useMemo(() => {
    const hasGoBackLink =
      appProps.referrer &&
      appProps.referrer.includes(`/token/${hash}`) &&
      !appProps.referrer.includes("instance");
  ...
  const tabs: Array<RoutedTab> = [
    ...
    shouldFetchHolders
      ? {
          ...
          title: "Holders",
          component: (
            <TokenHolders holdersQuery={holdersQuery} token={tokenQuery.data} />
          ),
        }
      : undefined,
    {
  ...
  const tokenTag = (
    <Tag isLoading={tokenInstanceQuery.isPlaceholderData}>
      {tokenQuery.data?.type}
    </Tag>
  );
  ...
  const appLink = (() => {
    ...
    try {
      const url = regexp.URL_PREFIX.test(
        tokenInstanceQuery.data.external_app_url
      )
      ...
      return (
        <LinkExternal
          href={tokenInstanceQuery.data.external_app_url}
          isLoading={isLoading}
          ml={{ base: 0, lg: "auto" }}
        >
          View in app
    ...
    <>
      ...
      {isLoading ? (
        ...
        <RoutedTabs
          tabs={tabs}
          tabListProps={
            isMobile ? { mt: 8 } : { mt: 3, py: 5, marginBottom: 0 }
          }
          rightSlot={
