...
const Validators = () => {
  ...
  const [statusFilter, setStatusFilter] = React.useState(
    ...
    ) as ValidatorsFilters["state_filter"]) || undefined
  );
  const [sort, setSort] = React.useState<ValidatorsSortingValue | undefined>(
    getSortValueFromQuery<ValidatorsSortingValue>(router.query, SORT_OPTIONS)
  );
  ...
  const {
    ...
    sorting: getSortParamsFromValue<
      ValidatorsSortingValue,
      ValidatorsSortingField,
      ValidatorsSorting["order"]
    >(sort),
    options: {
      enabled: config.features.validators.isEnabled,
      placeholderData: generateListStub<"validators">(VALIDATOR, 50, {
  ...
  const handleStateFilterChange = React.useCallback(
    (value: string | Array<string>) => {
      ...
      const state =
        value === "all"
          ? undefined
          : (value as ValidatorsFilters["state_filter"]);
  ...
  const sortButton = (
    <Sort options={SORT_OPTIONS} sort={sort} setSort={handleSortChange} />
  );
