...
test.describe("default view", () => {
  ...
  test.beforeEach(async ({ page, mount }) => {
    await page.route(buildApiUrl("stats"), (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify(statsMock.base),
      })
    );
    await page.route(buildApiUrl("homepage_blocks"), (route) =>
      route.fulfill({
        status: 200,
    ...
    await page.route(buildApiUrl("stats_charts_txs"), (route) =>
      route.fulfill({
        status: 200,
...
test.describe("custom hero plate background", () => {
  ...
  const extendedTest = test.extend({
    context: contextWithEnvs([
      {
        name: "NEXT_PUBLIC_HOMEPAGE_PLATE_BACKGROUND",
        value: `no-repeat center/cover url(${IMAGE_URL})`,
      },
