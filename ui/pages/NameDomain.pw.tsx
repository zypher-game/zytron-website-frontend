...
const DOMAIN_API_URL = buildApiUrl("domain_info", {
  chainId: config.chain.id,
  name: ensDomainMock.ensDomainA.name,
});
const DOMAIN_EVENTS_API_URL = buildApiUrl("domain_events", {
  chainId: config.chain.id,
  name: ensDomainMock.ensDomainA.name,
});
...
test("details tab", async ({ mount, page }) => {
  await page.route(DOMAIN_API_URL, (route) =>
    route.fulfill({
      status: 200,
...
test("history tab +@mobile", async ({ mount, page }) => {
  ...
  await page.route(DOMAIN_EVENTS_API_URL, (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify({
        items: [
