...
test.describe("search by name ", () => {
  ...
  extendedTest("+@mobile +@dark-mode", async ({ mount, page }) => {
    ...
    await page.route(buildApiUrl("search") + "?q=o", (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
          items: [
...
test("search by address hash +@mobile", async ({ mount, page }) => {
  ...
  await page.route(
    buildApiUrl("search") + `?q=${searchMock.address1.address}`,
    (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
...
test("search by block number +@mobile", async ({ mount, page }) => {
  ...
  await page.route(
    buildApiUrl("search") + `?q=${searchMock.block1.block_number}`,
    (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
...
test("search by block hash +@mobile", async ({ mount, page }) => {
  ...
  await page.route(
    buildApiUrl("search") + `?q=${searchMock.block1.block_hash}`,
    (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
...
test("search by tx hash +@mobile", async ({ mount, page }) => {
  ...
  await page.route(
    buildApiUrl("search") + `?q=${searchMock.tx1.tx_hash}`,
    (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
...
test("search by blob hash +@mobile", async ({ mount, page }) => {
  ...
  await page.route(
    buildApiUrl("search") + `?q=${searchMock.blob1.blob_hash}`,
    (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
...
testWithUserOps("search by user op hash +@mobile", async ({ mount, page }) => {
  ...
  await page.route(
    buildApiUrl("search") + `?q=${searchMock.userOp1.user_operation_hash}`,
    (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
...
test.describe("with apps", () => {
  ...
  extendedTest("default view +@mobile", async ({ mount, page }) => {
    ...
    await page.route(MARKETPLACE_CONFIG_URL, (route) =>
      route.fulfill({
        status: 200,
