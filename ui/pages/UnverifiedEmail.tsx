  ...
  const handleButtonClick = React.useCallback(async () => {
    ...
    try {
      ...
      const message = (() => {
        ...
        const payload = getErrorObjPayload<{
          seconds_before_next_resend: number;
        }>(error);
        if (!payload) {
          return;
        }
        ...
        const timeUntilNextResend = dayjs()
          .add(payload.seconds_before_next_resend, "seconds")
          .fromNow();
  ...
  return (
    <Box>
      ...
      <Text variant="secondary" mt={3}>
        <span>
          Please confirm your email address to use the My Account feature. A
