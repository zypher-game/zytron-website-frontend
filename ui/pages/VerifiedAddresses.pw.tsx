...
test.beforeEach(async ({ mockAssetResponse }) => {
  await mockAssetResponse(
    mocks.TOKEN_INFO_APPLICATION_BASE.iconUrl,
...
test("base view +@mobile", async ({ render, mockApiResponse }) => {
  await mockApiResponse(
    "verified_addresses",
    mocks.VERIFIED_ADDRESS_RESPONSE.DEFAULT,
    { pathParams: { chainId: "1" } }
  );
  await mockApiResponse(
    "token_info_applications",
    mocks.TOKEN_INFO_APPLICATIONS_RESPONSE.DEFAULT,
    { pathParams: { chainId: "1", id: undefined } }
  );
...
test("address verification flow", async ({ render, mockApiResponse, page }) => {
  ...
  await mockApiResponse(
    ...
    { pathParams: { chainId: "1", type: ":prepare" } }
  );
  await mockApiResponse(
    "address_verification",
    mocks.ADDRESS_VERIFY_RESPONSE.SUCCESS as never,
    { pathParams: { chainId: "1", type: ":verify" } }
  );
  await mockApiResponse("user_info", profileMock.base);
  ...
  // success screen
  await page
    .getByRole("button", { name: /view my verified addresses/i })
    .click();
...
test("application update flow", async ({ render, mockApiResponse, page }) => {
  ...
  await mockApiResponse(
    "token_info_applications",
    mocks.TOKEN_INFO_APPLICATIONS_RESPONSE.FOR_UPDATE,
    { pathParams: { chainId: "1", id: undefined } }
  );
  await mockApiResponse("user_info", profileMock.base);
  await mockApiResponse(
    "token_info_applications_config",
    mocks.TOKEN_INFO_FORM_CONFIG,
    { pathParams: { chainId: "1" } }
  );
  ...
  await mockApiResponse(
    ...
    mocks.TOKEN_INFO_APPLICATION.UPDATED_ITEM as never, // this mock is for PUT request
    {
      pathParams: {
        chainId: "1",
        id: mocks.TOKEN_INFO_APPLICATION.UPDATED_ITEM.id,
      },
    }
  );
  ...
  await page
    .locator("tr")
    .filter({ hasText: "waiting for update" })
