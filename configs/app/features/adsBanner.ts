...
const provider: AdBannerProviders = (() => {
  ...
  ) as AdBannerProviders;

  return envValue && SUPPORTED_AD_BANNER_PROVIDERS.includes(envValue)
    ? envValue
    : "slise";
})();

const additionalProvider = getEnvValue(
...
const config: Feature<AdsBannerFeaturePayload> = (() => {
  if (provider === "adbutler") {
    const desktopConfig = parseEnvJson<AdButlerConfig>(
      getEnvValue("NEXT_PUBLIC_AD_ADBUTLER_CONFIG_DESKTOP")
    );
    const mobileConfig = parseEnvJson<AdButlerConfig>(
      getEnvValue("NEXT_PUBLIC_AD_ADBUTLER_CONFIG_MOBILE")
    );
