...
function getSocketParams(router: NextRouter) {
  if (
    ...
    return {
      topic: "transactions:new_transaction" as const,
  ...
  if (
    ...
    return {
      topic: "transactions:new_pending_transaction" as const,
      event: "pending_transaction" as const,
    };
  }
...
function assertIsNewTxResponse(
  response: unknown
): response is { transaction: number } {
  return (
...
function assertIsNewPendingTxResponse(
  response: unknown
): response is { pending_transaction: number } {
  return (
    typeof response === "object" &&
    response !== null &&
    "pending_transaction" in response
  );
}
...
export default function useNewTxsSocket() {
  ...
  const handleNewTxMessage = React.useCallback(
    (
      response:
        | { transaction: number }
